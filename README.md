...
| **H3 Long AV Latent, footage prefix (Draken)** | Builds the long empty AV latent (snapped to `17k+5`, and to an audio-exact length by default). Optional `prefix_frames` + `prefix_audio` with `vae` + `audio_vae`: the last `prefix_context_frames` (default 39) of the footage are encoded into the head of the latent as a hard masked prefix, with optional feathering. Outputs the latent, the number of prefix frames to trim later, and an info string. |
